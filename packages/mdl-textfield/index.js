...
export default class MDLTextfield extends MDLComponent {
  static buildDom({
    id = `mdl-textfield-${++idCounter}`,
    helptextId = '',
    labelElement = null
  } = {}) {
    const root = document.createElement('div');
    const {ROOT: CSS_ROOT} = cssClasses;
    root.classList.add(CSS_ROOT);
    root.innerHTML = `
      <input type="text" class="${CSS_ROOT}__input" id="${id}"
             ${helptextId ? ` aria-controls="${helptextId}" ` : ''}>
    `;
    if (labelElement) {
      labelElement.classList.add(`${CSS_ROOT}__label`);
