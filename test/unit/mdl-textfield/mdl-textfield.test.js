...
test('buildDom creates a mdl-textfield component', t => {
  verifyBuildDom(MDLTextfield, t, bel`
...
test('buildDom auto-generates an input id when none is provided', t => {
  const root = MDLTextfield.buildDom();
  const {id} = root.querySelector('.mdl-textfield__input');
  t.true(/^mdl\-textfield\-\d$/.test(id), 'id matches mdl-textfield-<number>');
  t.end();
});

test('buildDom ensures auto-generated id counters are unique', t => {
  const {id: id1} = MDLTextfield.buildDom().querySelector('.mdl-textfield__input');
  const {id: id2} = MDLTextfield.buildDom().querySelector('.mdl-textfield__input');
  t.notEqual(id1, id2);
  t.end();
});

test('buildDom accepts a labelElement element option which is added as a textfield label', t => {
  verifyBuildDom(MDLTextfield, t, bel`
    <div class="mdl-textfield">
      <input type="text" class="mdl-textfield__input" id="my-textfield">
      <label class="mdl-textfield__label" for="my-textfield">My Textfield Label</label>
    </div>
  `, {id: 'my-textfield', labelElement: bel`<label>My Textfield Label</label>`});
  t.end();
});

test('buildDom sets aria-controls to the value of helptextId when given', t => {
  verifyBuildDom(MDLTextfield, t, bel`
    <div class="mdl-textfield">
      <input type="text" class="mdl-textfield__input" id="my-textfield" aria-controls="my-textfield-helptext">
    </div>
  `, {id: 'my-textfield', helptextId: 'my-textfield-helptext'});
  t.end();
});
...
test('#adapter.rmHelptextAttr does nothing if no help text element present', t => {
  const {component} = setupTest();
  t.doesNotThrow(() => component.getDefaultFoundation().adapter_.rmHelptextAttr('aria-label'));
  t.end();
});

test('#adapter.rmHelptextAttr removes an attribute on the help text element', t => {
  const {component} = setupTest();
  const helptext = getHelptext();
  helptext.setAttribute('aria-label', 'foo');
  component.helptextElement = helptext;
  component.getDefaultFoundation().adapter_.rmHelptextAttr('aria-label');
